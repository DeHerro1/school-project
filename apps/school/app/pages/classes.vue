...
import {
  Card, Button, Input, Label, Select, MultiSelect, Modal, Badge, Spinner,
...
const levelOptions = Object.values(ClassLevel).map((l) => ({ value: l, label: l }));
  ...
  <div>
    ...
    >
      <template v-if="isAdmin" #actions>
        <Button @click="showAdd = true"><Plus class="size-4" /> New class</Button>
      </template>
    </PageHeader>
      ...
      <Card class="hidden md:block">
        <Table>
          ...
          <TableBody>
            ...
            <TableRow v-if="!classes.length"><TableCell class="py-10 text-center text-muted-foreground">No classes yet.</TableCell></TableRow>
          </TableBody>
        </Table>
      </Card>
    </template>
    ...
    <Modal v-model:open="showAdd" title="New class">
      <form class="space-y-3" @submit.prevent="create">
        <div class="space-y-1.5"><Label>Name</Label><Input v-model="form.name" placeholder="e.g. Nursery A" required /></div>
        <div class="space-y-1.5"><Label>Level</Label><Select v-model="form.level" :options="levelOptions" /></div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="space-y-1.5"><Label>Number of students</Label><Input v-model="form.studentCount" type="number" min="0" placeholder="e.g. 25" /></div>
        </div>
    ...
    <Modal v-model:open="showEdit" :title="`Edit ${editing?.name ?? 'class'}`">
      <form class="space-y-3" @submit.prevent="saveEdit">
        ...
        <div class="space-y-1.5"><Label>Number of students</Label><Input v-model="editForm.studentCount" type="number" min="0" /></div>
